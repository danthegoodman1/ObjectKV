// This is a modified version of the FoundationDB tuple.go file.
// https://github.com/apple/foundationdb/blob/3f97fa046ba0/bindings/go/src/fdb/tuple/tuple.go
// FoundationDB is property of Apple Inc. and the FoundationDB project authors
...
//
// This orders by lexicographical order only. Example:
//
//	dir/
//	dir/a
//	dir/a/1
...
func fdbDecodeBytes(b []byte) ([]byte, int) {
	idx := findTerminator(b[1:])
	return bytes.Replace(b[1:idx+1], []byte{0x00, 0xFF}, []byte{0x00}, -1), idx + 2
}

func fdbDecodeString(b []byte) (string, int) {
	bp, idx := fdbDecodeBytes(b)
...
func decodeTuple(b []byte, nested bool) (Tuple, int, error) {
	...
	for i < len(b) {
		...
		case b[i] == bytesCode:
			el, off = fdbDecodeBytes(b[i:])
		case b[i] == stringCode:
			el, off = fdbDecodeString(b[i:])
...
// prefix).
func (t Tuple) RangeKeys() ([]byte, []byte) {
	p := t.Pack()
	return concat(p, 0x00), concat(p, 0xFF)
}
